		...
		*/
		...
		protected void afterHookedMethod(MethodHookParam param) throws Throwable {
			...
			try {  
				...
				XposedBridge.log("PresenceCheckWatchdogRunHook afterHookedMethod threw exception (1)" + e.getMessage().toString());
				e.printStackTrace();
			}  
			...
			if (context == null) {
				if (mDebugMode) Log.d(TAG,  "step-4 context == null");
				return;
			}
			...
			try {
				...
				if (packageManager != null) {
					...
					if (activities.size() > 0) {
						if (mDebugMode) Log.d(TAG, String.format("startActivity - android.nfc.action.TAG_LOST(%x%x%x%x)",
								uId[0], uId[1], uId[2], uId[3]));
						context.startActivity(intentToStart);
					} else {
						if (mDebugMode) Log.d(TAG,  String.format("activities.size() <= 0 (%x%x%x%x)",
								uId[0], uId[1], uId[2], uId[3]));
					}
				}
				...
				XposedBridge.log("PresenceCheckWatchdogRunHook afterHookedMethod threw exception (2)" + e.getMessage().toString());
				e.printStackTrace();
			}  
		}
	}
	...
	class NfcServiceOnRemoteEndpointDiscoveredHook extends XC_MethodHook {
		...
		protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
			try {
				...
				XposedBridge.log("NfcServiceOnRemoteEndpointDiscoveredHook beforeHookedMethod threw exception: " + e.getMessage().toString());
				e.printStackTrace();
			}  
		}
	}
		...
		if (lpparam.packageName.equals(Common.PACKAGE_NFC)) {
			...
			*/
			...
			XC_MethodHook initNfcServiceHook = new XC_MethodHook() {
				...
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					...
					try {
						...
						XposedBridge.log("Field mScreenStateHelper not found: " + e.getMessage().toString());
						e.printStackTrace();
					}
				}
			};
			...
			try {
				...
				XposedBridge.log("Method initSoundPool not found: " + e.getMessage().toString());
				e.printStackTrace();
			}
			...
			try {
				...
				XposedBridge.log("Method playSound not found: " + e.getMessage().toString());
				e.printStackTrace();
			}
			...
			XC_MethodHook checkScreenStateHook = new XC_MethodHook() {
				...
				protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
					...
					try {
						...
						XposedBridge.log("checkScreenStateHook beforeHookedMethod threw exception: " + e.getMessage().toString());
						e.printStackTrace();  
					}  
				}
			};
			...
			//	} catch (NoSuchMethodError e1) {
					try {
						...
						XposedBridge.log("Method checkScreenState not found: " + e2.getMessage().toString());
						e2.printStackTrace();
					}
			//	}
			//}
			...
			try {
					...
					findAndHookMethod(NfcService, "applyRouting", boolean.class, new XC_MethodHook() {
						...
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							...
							try {
								...
								XposedBridge.log("applyRouting beforeHookedMethod threw exception: " + e.getMessage().toString());
								e.printStackTrace();  
							}  
						}
						...
						protected void afterHookedMethod(MethodHookParam param) throws Throwable {
							try {
								...
								XposedBridge.log("applyRouting afterHookedMethod threw exception: " + e.getMessage().toString());
								e.printStackTrace();  
							}  
						}
					});
				...
				XposedBridge.log("Method applyRouting not found: " + e.getMessage().toString());
				e.printStackTrace();
			}
			...
			try {
				...
				XposedBridge.log("Method not found: " + e.getMessage().toString());
				e.printStackTrace();
			} catch (ClassNotFoundError e) {
				XposedBridge.log("Class not found: " + e.getMessage().toString());
				e.printStackTrace();
			}
